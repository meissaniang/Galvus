...
const SELECT_COLUMNS: &str =
    "id, name, hostname, port, username, identity_file, color, favorite";
...
pub fn create(conn: &Connection, input: &ServerInput) -> Result<Server, AppError> {
    conn.execute(
        "INSERT INTO servers (name, hostname, port, username, identity_file, color, favorite)
...
pub fn update(conn: &Connection, id: i64, input: &ServerInput) -> Result<Server, AppError> {
    conn.execute(
        "UPDATE servers
         SET name = ?1, hostname = ?2, port = ?3, username = ?4,
             identity_file = ?5, color = ?6, favorite = ?7
         WHERE id = ?8",
        params![
            input.name,
            input.hostname,

...
<template>
  <section class="page">
    <div class="toolbar">
      <div class="toolbar__search">
        ...
        <input
          v-model="query"
          type="text"
          placeholder="Rechercher une clé (nom, type, commentaire)…"
          spellcheck="false"
        />
      </div>
    ...
    <div v-else-if="filteredKeys.length === 0" class="state state--empty">
      <i class="pi pi-key" />
      <p v-if="keys.length === 0">Aucune clé détectée dans <code>~/.ssh</code>.</p>
      <p v-else>Aucune clé ne correspond à « {{ query }} ».</p>
    </div>

    <div v-else class="grid">
      <KeyCard v-for="item in filteredKeys" :key="item.path" :key-item="item" />
    </div>

    <p class="page__hint">
      <i class="pi pi-info-circle" /> Empreintes calculées via
      <code>ssh-keygen -lf</code>. Aucune clé privée n'est lue ni copiée.
    </p>
  </section>
</template>
